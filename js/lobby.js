/**
 * ?å°ç²¾æ? VR å±•é?ï¼šç’°?¯å?è¦?+ æ©Ÿå°ä»‹ç´¹ä¸»ç?å¼?
 */
...
const MEDIA_VERSION = 'hall250716d';
// åª’é?å¿«å??ˆæœ¬ï¼šæ›´?›è??¯å??–ç¸®?–å?èª¿é?æ­¤å€¼å³?¯å¼·?¶ç€è¦½?¨é??°è???
...
function buildThumbnailMenu() {
  ...
  thumbsEl.innerHTML = scenes.map((scene) => `
    <button type="button" class="lb-thumb" data-scene-id="${scene.id}" aria-label="?å? ${scene.title}">
...
async function switchScene(targetId) {
  ...
  try {
    ...
    if (loaderSubEl) loaderSubEl.textContent = `æ­?œ¨è¼‰å…¥ ${target.title}?¦`;
    ...
    console.error('[lobby-tour] ?‡æ??´æ™¯å¤±æ?', err);
...
function initViewer() {
  ...
  if (loaderSubEl) loaderSubEl.textContent = `æ­?œ¨è¼‰å…¥ ${first.title}?¦`;
...
async function boot() {
  if (loaderSubEl) loaderSubEl.textContent = 'æ­?œ¨è¼‰å…¥?¢å??§å®¹??;
  loaderEl?.classList.remove('is-hidden');
  try {
    await loadProductContent();
  } catch (err) {
    console.error('[lobby] ?¢å??§å®¹è¼‰å…¥å¤±æ?ï¼Œä??‹å? VR å±•é?', err);
  }
  initViewer();
}
